...
function Profile() {
    ...
    useEffect(() => {
        if (profileUser?.id) {
            ...
            console.log("Filtered posts for profile user:", filteredPosts); // Debugging log
    ...
    const handleSaveUser = (event) => {
        ...
        console.log("Saving updated user:", updatedUser); // Debugging log
        ...
        <div className={styles.profileContainer}>
            {isEditing ? (
                ...
                <>
                    <h2>{profileUser.name}'s Profile</h2>
                    <Description>{profileUser.name}'s Posts</Description>
                    {userPosts.length > 0 ? ( // Use local state here
